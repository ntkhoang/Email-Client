...
def load_read_ids(): # Load read email IDs from file
    if os.path.exists('read_ids.txt'):
        with open('read_ids.txt', 'r') as file:
            return file.read().splitlines()
    else:
        return []

def save_read_id(id): # Save read email ID to file
    with open('read_ids.txt', 'a') as file:
        file.write(id + '\n')

def list_emails(sock, downloaded_ids, read_ids): # List all emails on the server, with additional information about whether they are downloaded or read
    response = send_command(sock, 'UIDL\r\n')
    lines = response.split('\n')
    for line in lines[1:]:
        parts = line.split()
        if len(parts) == 2:
            status = 'đã đọc' if parts[1] in read_ids else 'chưa đọc'
            print(f'Email ID: {parts[1]}, Trạng thái: {status}')
...
if __name__ == "__main__":
    ...
    list_emails(sock, downloaded_ids, read_ids)
    ...
    if email_id not in downloaded_ids:
        ...
        save_read_id(email_id)  # Mark as read.
    else:
